...
class AccountMove(models.Model):
    ...
    l10n_bg_secure_sequence_number = fields.Integer(string="Inalteralbility No Gap Sequence #", readonly=True, copy=False)
    l10n_bg_hash = fields.Char(string="Inalterability Hash", readonly=True, copy=False)
    l10n_bg_string_to_hash = fields.Char(compute='_compute_string_to_hash', readonly=True, store=False)
    ...
    def _get_new_hash(self, secure_seq_number):
        ...
        prev_move = self.search([('state', '=', 'posted'),
                                 ...
                                 ('l10n_bg_secure_sequence_number', '!=', 0),
                                 ('l10n_bg_secure_sequence_number', '=', int(secure_seq_number) - 1)])
        ...
        return self._compute_hash(prev_move.l10n_bg_hash if prev_move else u'')

    def _compute_hash(self, previous_hash):
        """ Computes the hash of the browse_record given as self, based on the hash
        of the previous record in the company's securisation sequence given as parameter"""
        self.ensure_one()
        hash_string = sha256((previous_hash + self.l10n_bg_string_to_hash).encode('utf-8'))
    ...
    def _compute_string_to_hash(self):
        ...
        for move in self:
            ...
            move.l10n_bg_string_to_hash = dumps(values, sort_keys=True,
        ...
        for move in self:
            if move.company_id._is_accounting_unalterable():
                ...
                if (move.state == "posted" and set(vals).intersection(MOVE_FIELDS)):
                    raise UserError(_("According to the Bulgarian law, you cannot modify a journal entry in order for its posted data to be updated or deleted. Unauthorized field: %s.") % ', '.join(MOVE_FIELDS))
                # restrict the operation in case we are trying to overwrite existing hash
                if (move.l10n_bg_hash and 'l10n_bg_hash' in vals) or (move.l10n_bg_secure_sequence_number and 'l10n_bg_secure_sequence_number' in vals):
                    raise UserError(_('You cannot overwrite the values ensuring the inalterability of the accounting.'))
        res = super(AccountMove, self).write(vals)
        # write the hash and the secure_sequence_number when posting an account.move
        if has_been_posted:
            for move in self.filtered(lambda m: m.company_id._is_accounting_unalterable() and
                                                not (m.l10n_bg_secure_sequence_number or m.l10n_bg_hash)):
                new_number = move.company_id.l10n_bg_secure_sequence_id.next_by_id()
                vals_hashing = {'l10n_bg_secure_sequence_number': new_number,
                                'l10n_bg_hash': move._get_new_hash(new_number)}
    ...
    def _check_hash_integrity(self, company_id):
        ...
        moves = self.search([('state', '=', 'posted'),
                             ...
                             ('l10n_bg_secure_sequence_number', '!=', 0)],
                            order="l10n_bg_secure_sequence_number ASC")
        ...
        start_move_info = []
        for move in moves:
            if move.l10n_bg_hash != move._compute_hash(previous_hash=previous_hash):
            ...
            previous_hash = move.l10n_bg_hash
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(l.company_id._is_accounting_unalterable() and l.move_id.state == 'posted' for l in self):
                raise UserError(_("According to the Bulgarian law, you cannot modify a journal item in order for its posted data to be updated or deleted. Unauthorized field: %s.") % ', '.join(LINE_FIELDS))
...
class AccountJournal(models.Model):
    ...
    def _onchange_update_posted(self):
        if self.update_posted and self.company_id._is_accounting_unalterable():
            ...
            raise UserError(_("According to the Bulgarian law, you cannot modify a journal in order for its posted data to be updated or deleted. Unauthorized field: %s.") % field_string)
    ...
    def _is_journal_alterable(self):
        ...
        critical_domain = [('journal_id', '=', self.id),
                            '|', ('l10n_bg_hash', '!=', False),
                            '&', ('l10n_bg_secure_sequence_number', '!=', False),
                            ('l10n_bg_secure_sequence_number', '!=', 0)]
